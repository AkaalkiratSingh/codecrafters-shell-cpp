...
std::vector<str> split(const str &s, char delim) {
    ...
    str current;
    for (char c : s) {
        if (c == delim) { result.push_back(current); current.clear(); } else { current.push_back(c); }
    }
...
std::optional<str> find_in_path(const str &cmd) {
    for (const auto &dir : path_dirs()) {
        auto full = dir / cmd;
        if (isExecutable(full)) return full.string();
#ifdef _WIN32
        auto with_exe = full.string() + ".exe";
        std::filesystem::path exe_path{ with_exe };
        if (isExecutable(exe_path)) return with_exe;
#endif
    }
    return std::nullopt;
}
